...
void main() {
  group('IconFrame', () {
    ...
    test('a frame is not mistaken for a JSON message', () {
      ...
      expect(HotkeyPadMessage.decode(frame), isNull);
    });
  ...
  group('HotkeyPadMessage', () {
    test('round-trips every message shape', () {
      ...
      for (final message in messages) {
        final decoded = HotkeyPadMessage.decode(message.encode());
    ...
    test('returns null for malformed input', () {
      expect(HotkeyPadMessage.decode(utf8.encode('not json')), isNull);
      expect(HotkeyPadMessage.decode(const [0xff, 0xfe]), isNull);
    });

    test(
      'Hello defaults to an empty name and clientId rather than crashing',
      () {
        final decoded = HotkeyPadMessage.decode(utf8.encode('{"t":"hi"}'));
  ...
  group('DeckTheme', () {
    ...
    test('labels default to on when an older host omits them', () {
      ...
      final decoded =
          HotkeyPadMessage.decode(utf8.encode('{"t":"thm","v":"dark"}'))
              as SetAppearance?;
