 ...
 * mirrors whichever of [hideSystemBars]/[showSystemBars] Dart last asked
 * for — see [hideSystemBars]'s own doc comment for why that needs doing
 * natively rather than solely through Flutter's own `SystemChrome` call
 * (`main.dart`'s `hideSystemBars`/`showSystemBars`, kept for iOS and
 * pre-15 Android, where it still works fine on its own). */
class MainActivity : FlutterActivity() {
    // Dart's own last request, applied here too on every focus regain —
    // unconditionally hiding regardless of it would fight the portrait
    // case, where main.dart now explicitly asks for the bars back.
     ...
     * *visibility*: `WindowInsetsControllerCompat.hide`/`.show` still
     * actually hide/show them. What stopped working at that OS version is
     * Flutter's own `SystemChrome.setEnabledSystemUIMode(immersiveSticky)`,
     * built on the older system-UI-flags API edge-to-edge enforcement
     * overrides (see
    ...
    override fun configureFlutterEngine(flutterEngine: FlutterEngine) {
        ...
        MethodChannel(flutterEngine.dartExecutor.binaryMessenger, "hotkeypad/systembars")
            .setMethodCallHandler { call, result ->
                when (call.method) {
                    "setHidden" -> {
                        barsHidden = call.arguments as? Boolean ?: true
                        applySystemBars()
                        result.success(null)
                    }
                    else -> result.notImplemented()
